...
def main():
  ...
  try:
    base_url = config_file.get('comp_settings', 'url')
    ...
    if options.end_date is not None:
      end_time = datetime.strptime(options.end_date, '%Y-%m-%dT%H:%M:%S')
    else:
      end_time = datetime.utcnow()
    if options.start_date is not None:
      start_time = datetime.strptime(options.start_date, '%Y-%m-%dT%H:%M:%S')
    else:
